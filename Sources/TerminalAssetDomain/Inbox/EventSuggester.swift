...
public struct EventSuggestion: Sendable, Hashable {
    ...
    public enum Reason: Sendable, Hashable {
        case happeningNow
        case startsInMinutes(Int)
        case endedMinutesAgo(Int)
        case similarTopic
    }

    /// The signals behind the suggestion, strongest first (time, then topic). The app words them.
    public let reasons: [Reason]
    /// 0...1. Only suggestions at or above `EventSuggester.minimumConfidence` are returned.
    public let confidence: Double

    public init(eventKey: EventKey, eventTitle: String, reasons: [Reason], confidence: Double) {
...
public enum EventSuggester {
    ...
    public static func suggest(text: String, at moment: Date, events: [TimelineEvent]) -> EventSuggestion? {
        ...
        var best: (event: TimelineEvent, score: Double, reasons: [EventSuggestion.Reason])?
        ...
        for event in events where event.syncState == .active && !event.isAllDay {
            ...
            var reasons: [EventSuggestion.Reason] = []
            if let reason = temporal.reason { reasons.append(reason) }
            if lexical > 0 { reasons.append(.similarTopic) }
    ...
    private struct TemporalSignal {
        let value: Double
        let reason: EventSuggestion.Reason?

        static let none = TemporalSignal(value: 0, reason: nil)
    }
    ...
    private static func temporalScore(of event: TimelineEvent, at moment: Date) -> TemporalSignal {
        if event.startDate <= moment && moment < event.endDate {
            return TemporalSignal(value: 1, reason: .happeningNow)
        }
        if event.startDate > moment {
            ...
            return TemporalSignal(value: value, reason: .startsInMinutes(Int(minutes.rounded())))
        }
        let minutesAgo = moment.timeIntervalSince(event.endDate) / 60
        guard minutesAgo <= 30 else { return .none }
        return TemporalSignal(value: 0.5, reason: .endedMinutesAgo(Int(minutesAgo.rounded())))
    }
